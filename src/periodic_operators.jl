...
"""
@auto_hash_equals struct PeriodicDerivativeCoefficients{
    T,
    LowerOffset,
    UpperOffset,
    ExecutionMode,
    ...
    function PeriodicDerivativeCoefficients(
        ...
        accuracy_order::Int,
        source_of_coefficients::SourceOfCoefficients,
    ) where {T,LowerOffset,UpperOffset,ExecutionMode,SourceOfCoefficients}
...
function mul!(
    ...
    α,
    β,
) where {T,LowerOffset,UpperOffset}
    @boundscheck begin
    ...
    convolve_periodic_boundary_coefficients!(
        dest,
        lower_coef,
        central_coef,
        upper_coef,
...
@generated function convolve_periodic_boundary_coefficients!(
    dest::AbstractVector,
...
`mode=ThreadedMode())`.
"""
function periodic_central_derivative_coefficients(
    derivative_order,
    accuracy_order,
    T = Float64,
    mode = FastMode(),
...
mode=ThreadedMode()`.
"""
function periodic_derivative_coefficients(
    derivative_order,
    accuracy_order,
...
function periodic_derivative_coefficients(
    ...
    if derivative_order == 1
        if accuracy_order == 2
            ...
            elseif stencil_width == 11
                lower_coef =
                    SVector{5,T}([-42 // 512, -48 // 512, -27 // 512, -8 // 512, -1 // 512])
                upper_coef = -lower_coef
                central_coef = T(0)
            else
                method_exists = false
    ...
    if method_exists == false
        throw(
            ArgumentError(
                "Method with derivative_order=$derivative_order, accuracy_order=$accuracy_order, stencil_width=$stencil_width not implemented/derived.",
            ),
        )
    end
...
function periodic_derivative_coefficients(
    ...
    if derivative_order == 1
        if accuracy_order == 2
            ...
            elseif stencil_width == 11
                lower_coef =
                    SVector{5,T}([-1 // 110, -2 // 110, -3 // 110, -4 // 110, -5 // 110])
                upper_coef = -lower_coef
                central_coef = T(0)
            else
                method_exists = false
        ...
        elseif accuracy_order == 4
            ...
            elseif stencil_width == 9
                lower_coef =
                    SVector{4,T}([-126 // 1188, -193 // 1188, -142 // 1188, 86 // 1188])
                upper_coef = -lower_coef
                central_coef = T(0)
            elseif stencil_width == 11
                lower_coef = SVector{5,T}([
...
"""
@auto_hash_equals struct PeriodicDerivativeOperator{
    T,
    LowerOffset,
    UpperOffset,
    ExecutionMode,
    ...
    coefficients::PeriodicDerivativeCoefficients{
        T,
        LowerOffset,
        UpperOffset,
        ExecutionMode,
    ...
    function PeriodicDerivativeOperator(
        coefficients::PeriodicDerivativeCoefficients{
            T,
            LowerOffset,
            UpperOffset,
        ...
        grid_evaluate::Grid,
    ) where {T,LowerOffset,UpperOffset,ExecutionMode,SourceOfCoefficients,Grid}
        ...
        new{T,LowerOffset,UpperOffset,ExecutionMode,SourceOfCoefficients,Grid}(
            coefficients,
            grid_compute,
            grid_evaluate,
            Δx,
...
@inline source_of_coefficients(D::PeriodicDerivativeOperator) =
    source_of_coefficients(D.coefficients)


function Base.show(
    io::IO,
    D::PeriodicDerivativeOperator{T,LowerOffset,UpperOffset},
) where {T,LowerOffset,UpperOffset}
    x = D.grid_evaluate
...
`mode=ThreadedMode()`.
"""
function periodic_central_derivative_operator(
    derivative_order,
    accuracy_order,
    xmin,
    xmax,
    ...
    coefficients = periodic_central_derivative_coefficients(
        derivative_order,
        accuracy_order,
...
function periodic_derivative_operator(
    ...
    coefficients = periodic_derivative_coefficients(
        derivative_order,
        accuracy_order,
        left_offset,
        eltype(grid),
...
@inline function periodic_derivative_operator(
    derivative_order,
    accuracy_order,
    xmin,
    xmax,
...
"""
@auto_hash_equals struct PeriodicDissipationOperator{
    T,
    LowerOffset,
    UpperOffset,
    ExecutionMode,
    ...
    function PeriodicDissipationOperator(
        factor::T,
        Di::PeriodicDerivativeOperator{
            T,
            LowerOffset,
...
@auto_hash_equals struct PeriodicRationalDerivativeOperator{
    T<:Real,
    Dtype<:PeriodicDerivativeOperator{T},
    Nnum,
    Nden,
    RFFT,
    IRFFT,
    ...
    function PeriodicRationalDerivativeOperator(
        D::Dtype,
        num_coef::NTuple{Nnum,T},
        den_coef::NTuple{Nden,T},
        tmp::Vector{Complex{T}},
        ...
        rfft_plan::RFFT,
        irfft_plan::IRFFT,
    ) where {T<:Real,Dtype<:PeriodicDerivativeOperator{T},Nnum,Nden,RFFT,IRFFT}
        ...
        new{T,Dtype,Nnum,Nden,RFFT,IRFFT}(
            D,
            num_coef,
            den_coef,
            tmp,
...
function _eigvals!(
    eigval::Vector{Complex{T}},
    D::PeriodicDerivativeOperator{T},
) where {T<:Real}
    @unpack factor = D
...
function mul_poly(coef1, coef2)
    ...
    for idx1 = 0:length(coef1)-1, idx2 = 0:length(coef2)-1
        coef = Base.setindex(
            coef,
...
function Base.:+(
    rat1::PeriodicRationalDerivativeOperator,
