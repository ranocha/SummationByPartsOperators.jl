...
"""
@auto_hash_equals struct PeriodicDerivativeCoefficients{T, LowerOffset, UpperOffset,
                                                        ExecutionMode,
    ...
    function PeriodicDerivativeCoefficients(lower_coef::SVector{LowerOffset, T},
                                            central_coef::T,
                                            upper_coef::SVector{UpperOffset, T},
                                            mode::ExecutionMode, derivative_order::Int,
                                            accuracy_order::Int,
...
function mul!(dest::AbstractVector,
              ...
              u::AbstractVector, α, β) where {T, LowerOffset, UpperOffset}
    @boundscheck begin
...
function mul!(dest::AbstractVector,
              coefficients::PeriodicDerivativeCoefficients{T, LowerOffset, UpperOffset},
              u::AbstractVector, α) where {T, LowerOffset, UpperOffset}
    @boundscheck begin
        @argcheck length(dest)==length(u) DimensionMismatch
        @argcheck length(u)>LowerOffset + UpperOffset DimensionMismatch
    end
...
`mode=ThreadedMode())`.
"""
function periodic_central_derivative_coefficients(derivative_order, accuracy_order,
    ...
    PeriodicDerivativeCoefficients(lower_coef, central_coef, upper_coef, mode,
                                   derivative_order, accuracy_order, source)
end
...
mode=ThreadedMode()`.
"""
function periodic_derivative_coefficients(source::Holoborodko2008, derivative_order,
                                          accuracy_order;
                                          T = Float64, mode = FastMode(),
                                          stencil_width = accuracy_order + 3)
    method_exists = true
...
"""
@auto_hash_equals struct PeriodicDerivativeOperator{T, LowerOffset, UpperOffset,
                                                    ExecutionMode, SourceOfCoefficients,
                                                    Grid} <:
                         AbstractPeriodicDerivativeOperator{T}
    coefficients::PeriodicDerivativeCoefficients{T, LowerOffset, UpperOffset, ExecutionMode,
                                                 SourceOfCoefficients}
    ...
    function PeriodicDerivativeOperator(coefficients::PeriodicDerivativeCoefficients{T,
        ...
        grid_compute = range(grid_evaluate[1], stop = grid_evaluate[end - 1],
...
# Compute `α*D*u + β*dest` and store the result in `dest`.
Base.@propagate_inbounds function mul!(dest::AbstractVector, D::PeriodicDerivativeOperator,
...
"""
function integrate(func::Func, u::AbstractVector, D::PeriodicDerivativeOperator) where {Func}
    @boundscheck begin
...
@inline function periodic_derivative_operator(derivative_order, accuracy_order, xmin, xmax,
                                              N,
                                              mode,
                                              left_offset::Int = -(accuracy_order + 1) ÷ 2)
    if mode === Val(:serial) || mode === Val(:threads)
...
```
"""
function periodic_derivative_operator(source::Holoborodko2008, derivative_order,
                                      accuracy_order,
    ...
    grid = range(xmin, stop = xmax, length = N + 1) # N includes two identical boundary nodes
    coefficients = periodic_derivative_coefficients(source, derivative_order,
                                                    accuracy_order;
                                                    kwargs..., T = eltype(grid),
                                                    mode = mode)
    PeriodicDerivativeOperator(coefficients, grid)
end
...
"""
@auto_hash_equals struct PeriodicDissipationOperator{T, LowerOffset, UpperOffset,
                                                     ExecutionMode, SourceOfCoefficients,
                                                     Grid} <:
                         AbstractPeriodicDerivativeOperator{T}
    factor::T
    Di::PeriodicDerivativeOperator{T, LowerOffset, UpperOffset, ExecutionMode,
                                   SourceOfCoefficients, Grid}

    function PeriodicDissipationOperator(factor::T,
                                         Di::PeriodicDerivativeOperator{T, LowerOffset,
                                                                        UpperOffset,
                                                                        ExecutionMode,
                                                                        SourceOfCoefficients,
                                                                        Grid}) where {T,
...
function Base.show(io::IO, Di::PeriodicDissipationOperator)
    ...
    if get(io, :compact, false) == false
        print(io, " on a grid in [", first(Di.Di.grid_evaluate), ", ",
...
@auto_hash_equals struct PeriodicRationalDerivativeOperator{T <: Real,
    ...
    function PeriodicRationalDerivativeOperator(D::Dtype, num_coef::NTuple{Nnum, T},
                                                den_coef::NTuple{Nden, T},
                                                tmp::Vector{Complex{T}},
                                                eigval::Vector{Complex{T}}, rfft_plan::RFFT,
        ...
        new{T, Dtype, Nnum, Nden, RFFT, IRFFT}(D, num_coef, den_coef, tmp, eigval,
...
function PeriodicRationalDerivativeOperator(D::PeriodicDerivativeOperator{T}) where {T <:
    ...
    PeriodicRationalDerivativeOperator(D, (zero(T), one(T)), (one(T),), tmp, eigval,
                                       rfft_plan, irfft_plan)
end

function PeriodicRationalDerivativeOperator(rat::PeriodicRationalDerivativeOperator,
                                            num_coef, den_coef)
    PeriodicRationalDerivativeOperator(rat.D, num_coef, den_coef, rat.tmp, rat.eigval,
                                       rat.rfft_plan, rat.irfft_plan)
end

function PeriodicRationalDerivativeOperator(D::PeriodicDerivativeOperator, num_coef,
                                            den_coef)
    PeriodicRationalDerivativeOperator(PeriodicRationalDerivativeOperator(D), num_coef,
                                       den_coef)
end
...
function mul!(dest::AbstractVector{T}, rat::PeriodicRationalDerivativeOperator,
