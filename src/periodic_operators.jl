...
"""
@auto_hash_equals struct PeriodicDerivativeCoefficients{T,
                                                        LowerOffset,
                                                        UpperOffset,
                                                        ExecutionMode,
    ...
    function PeriodicDerivativeCoefficients(lower_coef::SVector{LowerOffset, T},
                                            central_coef::T,
                                            upper_coef::SVector{UpperOffset, T},
                                            mode::ExecutionMode,
                                            derivative_order::Int,
        ...
        new{T, LowerOffset, UpperOffset, ExecutionMode, SourceOfCoefficients}(lower_coef,
                                                                              central_coef,
                                                                              upper_coef,
                                                                              mode,
                                                                              derivative_order,
...
function mul!(dest::AbstractVector,
              coefficients::PeriodicDerivativeCoefficients{T, LowerOffset, UpperOffset},
              u::AbstractVector,
              α,
              β) where {T, LowerOffset, UpperOffset}
    @boundscheck begin
    ...
    convolve_periodic_boundary_coefficients!(dest,
                                             lower_coef,
                                             central_coef,
                                             upper_coef,
                                             u,
                                             α,
                                             β)
    convolve_interior_coefficients!(dest,
                                    lower_coef,
                                    central_coef,
                                    upper_coef,
                                    u,
                                    α,
                                    β,
                                    static_length(lower_coef),
                                    static_length(upper_coef),
                                    mode)
end
...
function mul!(dest::AbstractVector,
              coefficients::PeriodicDerivativeCoefficients{T, LowerOffset, UpperOffset},
              u::AbstractVector,
              α) where {T, LowerOffset, UpperOffset}
    @boundscheck begin
        @argcheck length(dest)==length(u) DimensionMismatch
        @argcheck length(u)>LowerOffset + UpperOffset DimensionMismatch
    end
...
`mode=ThreadedMode())`.
"""
function periodic_central_derivative_coefficients(derivative_order,
                                                  accuracy_order,
                                                  T = Float64,
                                                  mode = FastMode())
    ...
    PeriodicDerivativeCoefficients(lower_coef,
                                   central_coef,
                                   upper_coef,
                                   mode,
                                   derivative_order,
...
mode=ThreadedMode()`.
"""
function periodic_derivative_coefficients(derivative_order,
                                          accuracy_order,
...
mode=ThreadedMode()`.
"""
function periodic_derivative_coefficients(source::Holoborodko2008,
                                          derivative_order,
                                          accuracy_order;
                                          T = Float64,
                                          mode = FastMode(),
    ...
    if derivative_order == 1
        if accuracy_order == 2
            if stencil_width == 5
                lower_coef = SVector{2, T}([-2 // 8, -1 // 8])
                upper_coef = -lower_coef
                central_coef = T(0)
            elseif stencil_width == 7
                lower_coef = SVector{3, T}([-5 // 32, -4 // 32, -1 // 32])
                upper_coef = -lower_coef
                central_coef = T(0)
            elseif stencil_width == 9
                lower_coef = SVector{4, T}([-14 // 128, -14 // 128, -6 // 128, -1 // 128])
                upper_coef = -lower_coef
                central_coef = T(0)
        ...
        elseif accuracy_order == 4
            if stencil_width == 7
                lower_coef = SVector{3, T}([-39 // 96, -12 // 96, 5 // 96])
                upper_coef = -lower_coef
                central_coef = T(0)
            elseif stencil_width == 9
                lower_coef = SVector{4, T}([-27 // 96, -16 // 96, 1 // 96, 2 // 96])
                upper_coef = -lower_coef
                central_coef = T(0)
            elseif stencil_width == 11
                lower_coef = SVector{5, T}([
    ...
    elseif derivative_order == 2
        if accuracy_order == 2
            if stencil_width == 5
                lower_coef = SVector{2, T}([0, 1 // 4])
                upper_coef = lower_coef
                central_coef = T(-2 // 4)
            elseif stencil_width == 7
                lower_coef = SVector{3, T}([-1 // 16, 2 // 16, 1 // 16])
                upper_coef = lower_coef
                central_coef = T(-4 // 16)
            elseif stencil_width == 9
                lower_coef = SVector{4, T}([-4 // 64, 4 // 64, 4 // 64, 1 // 64])
                upper_coef = lower_coef
        ...
        elseif accuracy_order == 4
            if stencil_width == 7
                lower_coef = SVector{3, T}([1 // 12, 5 // 12, -1 // 12])
                upper_coef = lower_coef
                central_coef = T(-10 // 12)
            elseif stencil_width == 9
                lower_coef = SVector{4, T}([-12 // 192, 52 // 192, 12 // 192, -7 // 192])
                upper_coef = lower_coef
...
function periodic_derivative_coefficients(source::LanczosLowNoise,
    ...
    if derivative_order == 1
        if accuracy_order == 2
            if stencil_width == 5
                lower_coef = SVector{2, T}([-1 // 10, -2 // 10])
                upper_coef = -lower_coef
                central_coef = T(0)
            elseif stencil_width == 7
                lower_coef = SVector{3, T}([-1 // 28, -2 // 28, -3 // 28])
                upper_coef = -lower_coef
                central_coef = T(0)
            elseif stencil_width == 9
                lower_coef = SVector{4, T}([-1 // 60, -2 // 60, -3 // 60, -4 // 60])
                upper_coef = -lower_coef
                central_coef = T(0)
            elseif stencil_width == 11
                lower_coef = SVector{5, T}([
        ...
        elseif accuracy_order == 4
            if stencil_width == 7
                lower_coef = SVector{3, T}([-58 // 252, -67 // 252, 22 // 252])
                upper_coef = -lower_coef
                central_coef = T(0)
            elseif stencil_width == 9
...
"""
@auto_hash_equals struct PeriodicDerivativeOperator{T,
                                                    LowerOffset,
                                                    UpperOffset,
                                                    ExecutionMode,
                                                    SourceOfCoefficients,
    ...
    coefficients::PeriodicDerivativeCoefficients{T,
                                                 LowerOffset,
                                                 UpperOffset,
                                                 ExecutionMode,
                                                 SourceOfCoefficients}
    ...
    function PeriodicDerivativeOperator(coefficients::PeriodicDerivativeCoefficients{T,
                                                                                     LowerOffset,
                                                                                     UpperOffset,
                                                                                     ExecutionMode,
                                                                                     SourceOfCoefficients},
                                        grid_evaluate::Grid) where {T, LowerOffset,
                                                                    UpperOffset,
                                                                    ExecutionMode,
                                                                    SourceOfCoefficients,
                                                                    Grid}
        @argcheck length(grid_evaluate)>LowerOffset + UpperOffset DimensionMismatch
        grid_compute = range(grid_evaluate[1],
                             stop = grid_evaluate[end - 1],
                             length = length(grid_evaluate) - 1)

        Δx = step(grid_evaluate)
        factor = inv(Δx^coefficients.derivative_order)
        new{T, LowerOffset, UpperOffset, ExecutionMode, SourceOfCoefficients, Grid}(coefficients,
                                                                                    grid_compute,
                                                                                    grid_evaluate,
                                                                                    Δx,
                                                                                    factor)
...
function Base.show(io::IO,
                   D::PeriodicDerivativeOperator{T, LowerOffset, UpperOffset}) where {T,
                                                                                      LowerOffset,
                                                                                      UpperOffset
                                                                                      }
    x = D.grid_evaluate
    ...
    elseif derivative_order(D) == 3
        ...
        print(io, "Periodic ", derivative_order(D), "-derivative operator")
    end
    ...
    if get(io, :compact, false) == false
        ...
        print(io,
              ...
              " nodes to the left, ",
              UpperOffset,
              " nodes to the right, and coefficients")
    end
...
`mode=ThreadedMode()`.
"""
function periodic_central_derivative_operator(derivative_order,
                                              accuracy_order,
                                              xmin,
                                              xmax,
                                              N,
    ...
    if mode === Val(:serial) || mode === Val(:threads)
        ...
        Base.depwarn("Providing the argument `parallel` is deprecated." *
                     "Use `mode` instead.",
                     :periodic_central_derivative_operator)
        mode = _parallel_to_mode(mode)
    end
    grid_evaluate = range(xmin, stop = xmax, length = N + 1) # N includes two identical boundary nodes
    coefficients = periodic_central_derivative_coefficients(derivative_order,
                                                            accuracy_order,
                                                            eltype(grid_evaluate),
                                                            mode)
...
```
"""
function periodic_derivative_operator(derivative_order::Integer,
                                      accuracy_order,
                                      xmin,
                                      xmax,
                                      N,
    ...
    coefficients = periodic_derivative_coefficients(derivative_order,
                                                    accuracy_order,
                                                    left_offset,
                                                    eltype(grid),
                                                    mode)
...
@inline function periodic_derivative_operator(derivative_order,
                                              accuracy_order,
                                              xmin,
                                              xmax,
                                              N,
    ...
    periodic_derivative_operator(derivative_order,
                                 accuracy_order,
                                 xmin,
                                 xmax,
                                 N,
...
function periodic_derivative_operator(;
                                      derivative_order,
                                      accuracy_order,
                                      xmin,
                                      xmax,
...
```
"""
function periodic_derivative_operator(source::Holoborodko2008,
                                      derivative_order,
                                      accuracy_order,
                                      xmin,
                                      xmax,
    ...
    grid = range(xmin, stop = xmax, length = N + 1) # N includes two identical boundary nodes
    coefficients = periodic_derivative_coefficients(source,
                                                    derivative_order,
                                                    accuracy_order;
                                                    kwargs...,
                                                    T = eltype(grid),
...
function periodic_derivative_operator(source::Holoborodko2008;
    ...
    periodic_derivative_operator(source,
                                 derivative_order,
                                 accuracy_order,
                                 xmin,
                                 xmax,
...
`mode=ThreadedMode())`.
"""
function periodic_derivative_operator(derivative_order,
                                      accuracy_order,
...
"""
@auto_hash_equals struct PeriodicDissipationOperator{T,
                                                     LowerOffset,
                                                     UpperOffset,
                                                     ExecutionMode,
                                                     SourceOfCoefficients,
                         ...
                         AbstractPeriodicDerivativeOperator{T}
    factor::T
    Di::PeriodicDerivativeOperator{T,
                                   LowerOffset,
                                   UpperOffset,
                                   ExecutionMode,
                                   SourceOfCoefficients,
    ...
    function PeriodicDissipationOperator(factor::T,
                                         Di::PeriodicDerivativeOperator{T,
                                                                        LowerOffset,
                                                                        UpperOffset,
                                                                        ExecutionMode,
        ...
        new{T, LowerOffset, UpperOffset, ExecutionMode, SourceOfCoefficients, Grid}(factor,
                                                                                    Di)
    end
end
...
function Base.show(io::IO, Di::PeriodicDissipationOperator)
    if derivative_order(Di) == 2
        ...
        print(io, "Periodic ", derivative_order(Di), "-derivative dissipation operator")
    end
    print(io, " of order ", accuracy_order(Di))
    if get(io, :compact, false) == false
        print(io,
              " on a grid in [",
              first(Di.Di.grid_evaluate),
              ", ",
              last(Di.Di.grid_evaluate),
              "] using ",
              size(Di, 1),
              " nodes \n")
...
# Compute `α*D*u + β*dest` and store the result in `dest`.
Base.@propagate_inbounds function mul!(dest::AbstractVector,
                                       Di::PeriodicDissipationOperator,
                                       u::AbstractVector,
                                       α,
                                       β)
...
@auto_hash_equals struct PeriodicRationalDerivativeOperator{T <: Real,
                                                            Dtype <:
                                                            PeriodicDerivativeOperator{T},
                                                            Nnum,
                                                            Nden,
                                                            RFFT,
                                                            IRFFT} <:
    ...
    function PeriodicRationalDerivativeOperator(D::Dtype,
                                                num_coef::NTuple{Nnum, T},
                                                den_coef::NTuple{Nden, T},
                                                tmp::Vector{Complex{T}},
                                                eigval::Vector{Complex{T}},
                                                rfft_plan::RFFT,
                                                irfft_plan::IRFFT) where {T <: Real,
                                                                          Dtype <:
                                                                          PeriodicDerivativeOperator{T},
                                                                          Nnum, Nden, RFFT,
                                                                          IRFFT}
        ...
        new{T, Dtype, Nnum, Nden, RFFT, IRFFT}(D,
                                               num_coef,
                                               den_coef,
                                               tmp,
                                               eigval,
...
function _eigvals!(eigval::Vector{Complex{T}},
                   D::PeriodicDerivativeOperator{T}) where {T <: Real}
    @unpack factor = D
...
function PeriodicRationalDerivativeOperator(D::PeriodicDerivativeOperator{T}) where {T <:
                                                                                     Real}
    x = grid(D)
    u = zero.(x)
    ...
    PeriodicRationalDerivativeOperator(D,
                                       (zero(T), one(T)),
                                       (one(T),),
                                       tmp,
                                       eigval,
...
function PeriodicRationalDerivativeOperator(rat::PeriodicRationalDerivativeOperator,
                                            num_coef,
                                            den_coef)
    PeriodicRationalDerivativeOperator(rat.D,
                                       num_coef,
                                       den_coef,
                                       rat.tmp,
                                       rat.eigval,
...
function PeriodicRationalDerivativeOperator(D::PeriodicDerivativeOperator,
                                            num_coef,
                                            den_coef)
    PeriodicRationalDerivativeOperator(PeriodicRationalDerivativeOperator(D),
                                       num_coef,
                                       den_coef)
end
...
function mul_poly(coef1, coef2)
    ...
    for idx1 in 0:(length(coef1) - 1), idx2 in 0:(length(coef2) - 1)
        coef = Base.setindex(coef,
                             coef[idx1 + idx2 + 1] + coef1[idx1 + 1] * coef2[idx2 + 1],
                             idx1 + idx2 + 1)
    end
...
function Base.:+(rat1::PeriodicRationalDerivativeOperator,
                 rat2::PeriodicRationalDerivativeOperator)
    @argcheck rat1.D==rat2.D ArgumentError

    num_coef = add_poly(mul_poly(rat1.num_coef, rat2.den_coef),
...
function Base.:-(rat1::PeriodicRationalDerivativeOperator,
    ...
    num_coef = subtract_poly(mul_poly(rat1.num_coef, rat2.den_coef),
                             mul_poly(rat1.den_coef, rat2.num_coef))
    den_coef = mul_poly(rat1.den_coef, rat2.den_coef)
    PeriodicRationalDerivativeOperator(rat1, num_coef, den_coef)
end
...
function mul!(dest::AbstractVector{T},
              rat::PeriodicRationalDerivativeOperator,
...
function LinearAlgebra.ldiv!(dest::AbstractVector{T},
                             rat::PeriodicRationalDerivativeOperator,
                             u::AbstractVector{T}) where {T}
    @unpack D, num_coef, den_coef, tmp, eigval, rfft_plan, irfft_plan = rat
    N, _ = size(D)
