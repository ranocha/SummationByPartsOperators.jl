...
"""
@auto_hash_equals struct MultidimensionalMatrixDerivativeOperator{Dim, T, NodesType,
                                                                  DType <:
                                                                  AbstractMatrix{T},
    ...
    function MultidimensionalMatrixDerivativeOperator(nodes::NodesType,
                                                      ...
                                                      Ds::NTuple{Dim, DType},
                                                      accuracy_order::Int,
                                                      source::SourceOfCoefficients) where {
                                                                                           Dim,
                                                                                           T <:
                                                                                           Real,
                                                                                           NodesType,
        ...
        new{Dim, T, NodesType, DType, SourceOfCoefficients}(nodes, boundary_indices,
                                                            normals, weights,
                                                            weights_boundary, Ds,
...
"""
function integrate_boundary(func, u, D::AbstractMultidimensionalMatrixDerivativeOperator,
                            dim)
    return integrate(func, restrict_boundary(u, D), weights_boundary_scaled(D, dim))
end

function integrate_boundary(u, D::AbstractMultidimensionalMatrixDerivativeOperator, dim)
    integrate_boundary(identity, u, D, dim)
end

function weights_boundary(D::AbstractMultidimensionalMatrixDerivativeOperator)
    get_weight_boundary.(Ref(D), 1:length(D.weights_boundary))
end
function weights_boundary_scaled(D::AbstractMultidimensionalMatrixDerivativeOperator,
                                 dim::Int)
...
function get_weight_boundary_scaled(D::AbstractMultidimensionalMatrixDerivativeOperator,
                                    dim::Int, i::Int)
    @unpack normals = D
...
function Base.show(io::IO, D::MultidimensionalMatrixDerivativeOperator)
    if get(io, :compact, false)
        ...
        x = grid(D)
        print(io, ndims(D), "-dimensional matrix-based first-derivative operator {T=",
              eltype(D), "}")
