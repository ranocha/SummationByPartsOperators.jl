...
strength.
"""
@auto_hash_equals struct ConstantFilter{T <: Real, Nodal2Modal, Modal2Nodal, Tmp,
    ...
    function ConstantFilter(coefficients::Vector{T}, nodal2modal::Nodal2Modal,
                            modal2nodal::Modal2Nodal, tmp::Tmp,
                            filter::FilterFunction) where {T <: Real, Nodal2Modal,
                                                           Modal2Nodal, Tmp, FilterFunction}
        @argcheck length(coefficients) == size(tmp, 1)

        new{T, Nodal2Modal, Modal2Nodal, Tmp, FilterFunction}(coefficients, nodal2modal,
                                                              modal2nodal, tmp, filter)
    end
end
