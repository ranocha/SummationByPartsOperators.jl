...
"""
@auto_hash_equals struct DerivativeCoefficients{T, LeftBoundary, RightBoundary,
                                                LeftBoundaryDerivatives,
                                                RightBoundaryDerivatives,
                                                LowerOffset, UpperOffset, LeftWidth,
                                                RightWidth, ExecutionMode,
    ...
    function DerivativeCoefficients(left_boundary::LeftBoundary,
                                    right_boundary::RightBoundary,
                                    left_boundary_derivatives::LeftBoundaryDerivatives,
                                    right_boundary_derivatives::RightBoundaryDerivatives,
                                    lower_coef::SVector{LowerOffset, T}, central_coef::T,
                                    upper_coef::SVector{UpperOffset, T},
                                    left_weights::SVector{LeftWidth, T},
                                    right_weights::SVector{RightWidth, T},
                                    mode::ExecutionMode, derivative_order::Int,
                                    accuracy_order::Int,
                                    source_of_coefficients::SourceOfCoefficients) where {T,
                                                                                         LeftBoundary,
                                                                                         RightBoundary,
                                                                                         LeftBoundaryDerivatives,
                                                                                         RightBoundaryDerivatives,
        ...
        new{T, LeftBoundary, RightBoundary, LeftBoundaryDerivatives,
            RightBoundaryDerivatives, LowerOffset, UpperOffset, LeftWidth, RightWidth,
            ExecutionMode, SourceOfCoefficients}(left_boundary, right_boundary,
                                                 ...
                                                 symmetric, source_of_coefficients)
    end
end
...
function mul!(dest::AbstractVector, coefficients::DerivativeCoefficients, u::AbstractVector,
    ...
    @boundscheck begin
        @argcheck length(dest)==length(u) DimensionMismatch
        @argcheck length(u)>length(lower_coef) + length(upper_coef) DimensionMismatch
        @argcheck length(u)>length(coefficients.left_boundary) +
...
function Base.:-(coef_row::DerivativeCoefficientRow{T, Start, Length}) where {T, Start,
                                                                              Length}
    DerivativeCoefficientRow{T, Start, Length}(-coef_row.coef)
end

function widening_plus(row1::SVector{Length1, T},
                       row2::SVector{Length2, T}) where {T, Length1, Length2}
...
function Base.:+(coef_row1::DerivativeCoefficientRow{T, Start1, Length1},
                 coef_row2::DerivativeCoefficientRow{T, Start2, Length2}) where {T, Start1,
                                                                                 Length1,
                                                                                 Start2,
                                                                                 Length2}
...
function Base.:/(coef_row::DerivativeCoefficientRow{T, Start, Length},
                 α::Number) where {T, Start, Length}
    DerivativeCoefficientRow{T, Start, Length}(coef_row.coef / α)
end

@inline function convolve_left_row(coef_row::DerivativeCoefficientRow{T, Start, Length}, u,
                                   ::SafeMode) where {T, Start, Length}
...
@inline function convolve_left_row(coef_row::DerivativeCoefficientRow{T, Start, Length}, u,
                                   ::Union{FastMode, ThreadedMode}) where {T, Start, Length}
    @inbounds tmp = coef_row.coef[1] * u[Start]
    @inbounds for i in 2:Length
...
@inline function convolve_right_row(coef_row::DerivativeCoefficientRow{T, Start, Length}, u,
                                    ::SafeMode) where {T, Start, Length}
...
@unroll function convolve_boundary_coefficients!(dest::AbstractVector, left_boundary,
                                                 right_boundary, u::AbstractVector, α, β,
                                                 mode::SafeMode)
...
@unroll function convolve_boundary_coefficients!(dest::AbstractVector, left_boundary,
                                                 right_boundary, u::AbstractVector, α,
                                                 mode::SafeMode)
    @unroll for i in 1:length(left_boundary)
        tmp = convolve_left_row(left_boundary[i], u, mode)
...
"""
@auto_hash_equals struct DerivativeOperator{T, LeftBoundary, RightBoundary,
                                            LeftBoundaryDerivatives,
                                            RightBoundaryDerivatives,
                                            LowerOffset, UpperOffset, LeftWidth, RightWidth,
                                            ExecutionMode, SourceOfCoefficients, Grid} <:
                         AbstractNonperiodicDerivativeOperator{T}
    coefficients::DerivativeCoefficients{T, LeftBoundary, RightBoundary,
                                         LeftBoundaryDerivatives, RightBoundaryDerivatives,
                                         LowerOffset, UpperOffset, LeftWidth, RightWidth,
    ...
    function DerivativeOperator(coefficients::DerivativeCoefficients{T, LeftBoundary,
                                                                     RightBoundary,
                                                                     LeftBoundaryDerivatives,
                                                                     RightBoundaryDerivatives,
                                                                     LowerOffset,
                                                                     UpperOffset, LeftWidth,
                                                                     RightWidth,
                                                                     ExecutionMode,
                                                                     SourceOfCoefficients},
                                grid::Grid) where {T, LeftBoundary, RightBoundary,
                                                   LeftBoundaryDerivatives,
                                                   RightBoundaryDerivatives,
                                                   LowerOffset, UpperOffset, LeftWidth,
                                                   RightWidth, ExecutionMode,
        ...
        @argcheck length(grid)>length(coefficients.left_boundary) +
                               length(coefficients.right_boundary) DimensionMismatch
        ...
        new{T, LeftBoundary, RightBoundary, LeftBoundaryDerivatives,
            RightBoundaryDerivatives, LowerOffset, UpperOffset, LeftWidth, RightWidth,
            ExecutionMode, SourceOfCoefficients, Grid}(coefficients, grid, Δx, factor)
    end
end
...
@inline function derivative_left(D::AbstractNonperiodicDerivativeOperator, u,
                                 der_order::Val{0})
    u[begin]
end
...
@inline function derivative_right(D::DerivativeOperator, u, der_order::Val{N}) where {N}
    convolve_right_row(D.coefficients.right_boundary_derivatives[N], u,
                       D.coefficients.mode) / D.Δx^N
end

@inline function derivative_right(D::AbstractNonperiodicDerivativeOperator, u,
                                  der_order::Val{0})
    u[end]
end
...
"""
@inline function mul_transpose_derivative_left!(u::AbstractVector, D::DerivativeOperator,
                                                der_order::Val{N}, α = true,
                                                β = false) where {N}
    factor = α / D.Δx^N
...
@inline function mul_transpose_derivative_left!(u::AbstractVector, D::DerivativeOperator,
                                                der_order::Val{0}, α = true, β = false)
...
Thus, the coefficients `α, β` have the same meaning as in [`mul!`](@ref).
"""
@inline function mul_transpose_derivative_right!(u::AbstractVector, D::DerivativeOperator,
                                                 der_order::Val{N}, α = true,
                                                 β = false) where {N}
    factor = α / D.Δx^N
...
# Compute `α*D*u + β*dest` and store the result in `dest`.
Base.@propagate_inbounds function mul!(dest::AbstractVector, D::DerivativeOperator,
                                       u::AbstractVector, α, β)
    @boundscheck begin
...
function derivative_operator(source_of_coefficients, derivative_order, accuracy_order,
    ...
    if derivative_order == 1
        coefficients = first_derivative_coefficients(source_of_coefficients, accuracy_order,
                                                     eltype(grid), mode)
    elseif derivative_order == 2
        coefficients = second_derivative_coefficients(source_of_coefficients,
                                                      accuracy_order, eltype(grid), mode)
    elseif derivative_order == 3
        coefficients = third_derivative_coefficients(source_of_coefficients, accuracy_order,
                                                     eltype(grid), mode)
    elseif derivative_order == 4
        coefficients = fourth_derivative_coefficients(source_of_coefficients,
                                                      accuracy_order, eltype(grid), mode)
    else
...
@inline construct_grid(source_of_coefficients, accuracy_order, xmin, xmax, N) = range(xmin,
                                                                                      stop = xmax,
                                                                                      length = N)
