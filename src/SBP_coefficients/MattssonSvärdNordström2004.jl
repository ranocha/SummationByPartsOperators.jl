...
function dissipation_coefficients(source::MattssonSvärdNordström2004,
                                  order::Int,
                                  grid,
                                  left_weights,
                                  right_weights,
    ...
    if order == 2
        coefficient_cache = MattssonSvärdNordström2004Cache2(inv_left_weights,
                                                             inv_right_weights)
        b = fill(one(T), length(grid))
        b[1] = T(0)
    elseif order == 4
        coefficient_cache = MattssonSvärdNordström2004Cache4(inv_left_weights,
                                                             inv_right_weights)
        b = fill(one(T), length(grid))
        b[1] = b[end] = T(0)
    elseif order == 6
        coefficient_cache = MattssonSvärdNordström2004Cache6(inv_left_weights,
                                                             inv_right_weights)
        b = fill(one(T), length(grid))
        b[1] = b[2] = b[end] = T(0)
    elseif order == 8
        coefficient_cache = MattssonSvärdNordström2004Cache8(inv_left_weights,
                                                             inv_right_weights)
        b = fill(one(T), length(grid))
...
struct MattssonSvärdNordström2004Cache2{T, LeftWidth, RightWidth} <:
       AbstractCoefficientCache{T}
    inv_left_weights::SVector{LeftWidth, T}
    inv_right_weights::SVector{RightWidth, T}

    function MattssonSvärdNordström2004Cache2(inv_left_weights::SVector{LeftWidth, T},
                                              inv_right_weights::SVector{RightWidth, T}) where {
                                                                                                T,
                                                                                                LeftWidth,
                                                                                                RightWidth
                                                                                                }
        ...
        if length(inv_left_weights) < boundary_length
            inv_left_weights = SVector(inv_left_weights...,
                                       ntuple(j -> one(T),
                                              boundary_length - length(inv_left_weights))...)
        end
        if length(inv_right_weights) < boundary_length
            inv_right_weights = SVector(inv_right_weights...,
                                        ntuple(j -> one(T),
                                               boundary_length - length(inv_right_weights))...)
        end

        new{T, length(inv_left_weights), length(inv_right_weights)}(inv_left_weights,
                                                                    inv_right_weights)
    end
end
...
function convolve_boundary_coefficients!(dest::AbstractVector,
                                         cache::MattssonSvärdNordström2004Cache2,
                                         u::AbstractVector,
                                         b::AbstractVector,
                                         α,
                                         ::SafeMode)
    ...
    @inbounds begin
        ...
        dest[2] = α *
                  inv_left_weights[2] *
                  ((b[1] - b[2]) * u[1] + (b[1] + b[2] + b[3]) * u[2] - b[3] * u[3])
        for i in 3:length(inv_left_weights)
            dest[i] = α *
                      inv_left_weights[i] *
                      (-b[i] * u[i - 1] + (b[i] + b[i + 1]) * u[i] - b[i + 1] * u[i + 1])
        end

        dest[end] = α * inv_right_weights[1] * (b[end] * u[end] - b[end] * u[end - 1])
        dest[end - 1] = α *
                        inv_right_weights[2] *
                        (-b[end - 1] * u[end - 2] + (b[end - 1] + b[end]) * u[end - 1] -
                         b[end] * u[end])
        for i in 2:(length(inv_right_weights) - 1)
            dest[end - i] = α *
                            inv_right_weights[i + 1] *
                            (-b[end - i] * u[end - (i - 1)] +
    ...
    @inbounds begin
        ...
        for i in 2:(length(inv_right_weights) - 1)
            dest[end - i] = α *
                            ...
                            (-b[end - i] * u[end - (i - 1)] +
                             (b[end - i] + b[end - (i + 1)]) * u[end - i] -
                             b[end - (i + 1)] * u[end - (i + 1)]) + β * dest[end - i]
        end
    end
end
function convolve_boundary_coefficients!(dest::AbstractVector,
...
@inline function convolve_interior_coefficients_loopbody(i,
                                                         cache::MattssonSvärdNordström2004Cache2,
                                                         u,
                                                         b)
    @inbounds begin
...
struct MattssonSvärdNordström2004Cache4{T, LeftWidth, RightWidth} <:
       AbstractCoefficientCache{T}
    inv_left_weights::SVector{LeftWidth, T}
    inv_right_weights::SVector{RightWidth, T}

    function MattssonSvärdNordström2004Cache4(inv_left_weights::SVector{LeftWidth, T},
                                              inv_right_weights::SVector{RightWidth, T}) where {
                                                                                                T,
                                                                                                LeftWidth,
                                                                                                RightWidth
...
function convolve_boundary_coefficients!(dest::AbstractVector,
                                         cache::MattssonSvärdNordström2004Cache4,
                                         u::AbstractVector,
                                         b::AbstractVector,
                                         α,
                                         ::SafeMode)
    ...
    @inbounds begin
        dest[1] = α *
                  inv_left_weights[1] *
                  (-(b[1] + b[2]) * u[1] + 2 * (b[1] + b[2]) * u[2] - (b[1] + b[2]) * u[3])
        dest[2] = α *
                  inv_left_weights[2] *
                  (2 * (b[1] + b[2]) * u[1] - (4 * b[1] + 4 * b[2] + b[3]) * u[2] +
                   2 * (b[1] + b[2] + b[3]) * u[3] - b[3] * u[4])
        dest[3] = α *
                  inv_left_weights[3] *
                  (-(b[1] + b[2]) * u[1] + 2 * (b[1] + b[2] + b[3]) * u[2] -
                   (b[1] + b[2] + 4 * b[3] + b[4]) * u[3] + 2 * (b[3] + b[4]) * u[4] -
                   b[4] * u[5])
        for i in 4:length(inv_left_weights)
            dest[i] = α *
                      inv_left_weights[i] *
                      (-b[i - 1] * u[i - 2] + 2 * (b[i - 1] + b[i]) * u[i - 1] -
...
function convolve_boundary_coefficients!(dest::AbstractVector,
    ...
    @inbounds begin
        ...
        dest[2] = α *
                  inv_left_weights[2] *
                  (2 * (b[1] + b[2]) * u[1] - (4 * b[1] + 4 * b[2] + b[3]) * u[2] +
                   2 * (b[1] + b[2] + b[3]) * u[3] - b[3] * u[4]) + β * dest[2]
        dest[3] = α *
                  inv_left_weights[3] *
                  (-(b[1] + b[2]) * u[1] + 2 * (b[1] + b[2] + b[3]) * u[2] -
                   (b[1] + b[2] + 4 * b[3] + b[4]) * u[3] + 2 * (b[3] + b[4]) * u[4] -
        ...
        dest[end] = α *
                    ...
                    (-(b[end] + b[end - 1]) * u[end] +
                     2 * (b[end] + b[end - 1]) * u[end - 1] -
                     (b[end] + b[end - 1]) * u[end - 2]) + β * dest[end]
        dest[end - 1] = α *
                        inv_right_weights[2] *
                        (+2 * (b[end] + b[end - 1]) * u[end] -
                         (b[end - 2] + 4 * b[end - 1] + 4 * b[end]) * u[end - 1] +
                         2 * (b[end - 2] + b[end - 1] + b[end]) * u[end - 2] -
                         b[end - 2] * u[end - 3]) + β * dest[end - 1]
        dest[end - 2] = α *
                        inv_right_weights[3] *
                        (-(b[end - 1] + b[end]) * u[end] +
                         2 * (b[end - 2] + b[end - 1] + b[end]) * u[end - 1] -
        ...
        for i in 3:(length(inv_right_weights) - 1)
            dest[end - i] = α *
                            ...
                            (-b[end - (i - 1)] * u[end - (i - 2)] +
                             ...
                             u[end - i] +
                             2 * (b[end - i] + b[end - (i + 1)]) * u[end - (i + 1)] -
                             b[end - (i + 1)] * u[end - (i + 2)]) + β * dest[end - i]
        end
    end
end
function convolve_boundary_coefficients!(dest::AbstractVector,
...
@inline function convolve_interior_coefficients_loopbody(i,
    ...
    @inbounds begin
        ...
        retval = (-b_im1 * u[i - 2] + 2 * (b_im1 + b_i) * u[i - 1] -
                  (b_im1 + 4b_i + b_ip1) * u[i] + 2 * (b_i + b_ip1) * u[i + 1] -
                  b_ip1 * u[i + 2])
    end
...
struct MattssonSvärdNordström2004Cache6{T, LeftWidth, RightWidth} <:
       AbstractCoefficientCache{T}
    inv_left_weights::SVector{LeftWidth, T}
    inv_right_weights::SVector{RightWidth, T}

    function MattssonSvärdNordström2004Cache6(inv_left_weights::SVector{LeftWidth, T},
                                              inv_right_weights::SVector{RightWidth, T}) where {
                                                                                                T,
                                                                                                LeftWidth,
                                                                                                RightWidth
...
function convolve_boundary_coefficients!(dest::AbstractVector,
                                         cache::MattssonSvärdNordström2004Cache6,
                                         u::AbstractVector,
                                         b::AbstractVector,
                                         α,
                                         ::SafeMode)
    ...
    @inbounds begin
        dest[1] = α *
                  inv_left_weights[1] *
                  (-(b[1] + b[2] + b[3]) * u[1] + 3 * (b[1] + b[2] + b[3]) * u[2] -
                   3 * (b[1] + b[2] + b[3]) * u[3] + (b[1] + b[2] + b[3]) * u[4])
        dest[2] = α *
                  inv_left_weights[2] *
                  (3 * (b[1] + b[2] + b[3]) * u[1] -
                   (9 * b[1] + 9 * b[2] + 9 * b[3] + b[4]) * u[2] +
                   3 * (3 * b[1] + 3 * b[2] + 3 * b[3] + b[4]) * u[3] -
                   3 * (b[1] + b[2] + b[3] + b[4]) * u[4] + b[4] * u[5])
        dest[3] = α *
                  inv_left_weights[3] *
                  (-3 * (b[1] + b[2] + b[3]) * u[1] +
        ...
        dest[end - 2] = α *
                        ...
                        (-3 * (b[end] + b[end - 1]) * u[end] +
                         ...
                         (b[end - 3] + 9 * b[end - 2] + 9 * b[end - 1] + 9 * b[end]) *
                         u[end - 2] +
                         3 * (b[end - 3] + 3 * b[end - 2] + b[end - 1] + b[end]) *
                         u[end - 3] -
        ...
        dest[end - 3] = α *
                        ...
                        ((b[end] + b[end - 1]) * u[end] -
                         ...
                         3 * (b[end - 3] + 3 * b[end - 2] + b[end - 1] + b[end]) *
                         u[end - 2] -
                         (b[end - 4] + 9 * b[end - 3] + 9 * b[end - 2] + b[end - 1] +
                          b[end]) * u[end - 3] +
        ...
        for i in 4:(length(inv_right_weights) - 1)
            dest[end - i] = α *
                            ...
                            (b[end - (i + 1)] * u[end - (i + 3)] -
                             ...
                             3 * (b[end - (i + 1)] + 3 * b[end - i] + b[end - (i - 1)]) *
                             u[end - (i + 1)] -
                             (b[end - (i + 1)] + 9 * b[end - i] + 9 * b[end - (i - 1)] +
                              b[end - (i - 2)]) *
                             u[end - i] +
                             3 * (b[end - i] + 3 * b[end - (i - 1)] + b[end - (i - 2)]) *
                             u[end - (i - 1)] -
    ...
    @inbounds begin
        ...
        dest[2] = α *
                  ...
                  (3 * (b[1] + b[2] + b[3]) * u[1] -
                   (9 * b[1] + 9 * b[2] + 9 * b[3] + b[4]) * u[2] +
                   3 * (3 * b[1] + 3 * b[2] + 3 * b[3] + b[4]) * u[3] -
                   3 * (b[1] + b[2] + b[3] + b[4]) * u[4] + b[4] * u[5]) + β * dest[2]
        dest[3] = α *
                  inv_left_weights[3] *
                  (-3 * (b[1] + b[2] + b[3]) * u[1] +
                   3 * (3 * b[1] + 3 * b[2] + 3 * b[3] + b[4]) * u[2] -
                   (9 * b[1] + 9 * b[2] + 9 * b[3] + 9 * b[4] + b[5]) * u[3] +
                   3 * (b[1] + b[2] + b[3] + 3 * b[4] + b[5]) * u[4] -
                   3 * (b[4] + b[5]) * u[5] + b[5] * u[6]) + β * dest[3]
        dest[4] = α *
                  inv_left_weights[4] *
                  ((b[1] + b[2] + b[3]) * u[1] - 3 * (b[1] + b[2] + b[3] + b[4]) * u[2] +
                   3 * (b[1] + b[2] + b[3] + 3 * b[4] + b[5]) * u[3] -
        ...
        dest[end] = α *
                    ...
                    (-(b[end] + b[end - 1]) * u[end] +
                     3 * (b[end] + b[end - 1]) * u[end - 1] -
                     3 * (b[end] + b[end - 1]) * u[end - 2] +
                     (b[end] + b[end - 1]) * u[end - 3]) + β * dest[end]
        dest[end - 1] = α *
                        inv_right_weights[2] *
                        (3 * (b[end] + b[end - 1]) * u[end] -
                         (b[end - 2] + 9 * b[end - 1] + 9 * b[end]) * u[end - 1] +
        ...
        for i in 4:(length(inv_right_weights) - 1)
            dest[end - i] = α *
                            ...
                            (b[end - (i + 1)] * u[end - (i + 3)] -
                             ...
                             u[end - (i - 1)] -
                             3 * (b[end - (i - 1)] + b[end - (i - 2)]) * u[end - (i - 2)] +
                             b[end - (i - 2)] * u[end - (i - 3)]) + β * dest[end - i]
        end
    end
end
function convolve_boundary_coefficients!(dest::AbstractVector,
...
@inline function convolve_interior_coefficients_loopbody(i,
    ...
    @inbounds begin
        ...
        retval = (b_im1 * u[i - 3] - 3 * (b_im1 + b_i) * u[i - 2] +
...
struct MattssonSvärdNordström2004Cache8{T, LeftWidth, RightWidth} <:
       AbstractCoefficientCache{T}
    inv_left_weights::SVector{LeftWidth, T}
    inv_right_weights::SVector{RightWidth, T}

    function MattssonSvärdNordström2004Cache8(inv_left_weights::SVector{LeftWidth, T},
                                              inv_right_weights::SVector{RightWidth, T}) where {
                                                                                                T,
                                                                                                LeftWidth,
                                                                                                RightWidth
...
function convolve_boundary_coefficients!(dest::AbstractVector,
                                         cache::MattssonSvärdNordström2004Cache8,
                                         u::AbstractVector,
                                         b::AbstractVector,
                                         α,
                                         ::SafeMode)
    ...
    @inbounds begin
        dest[1] = α *
                  inv_left_weights[1] *
                  (-(b[1] + b[2] + b[3]) * u[1] + 4 * (b[1] + b[2] + b[3]) * u[2] -
                   6 * (b[1] + b[2] + b[3]) * u[3] + 4 * (b[1] + b[2] + b[3]) * u[4] -
                   (b[1] + b[2] + b[3]) * u[5])
        dest[2] = α *
                  inv_left_weights[2] *
                  (4 * (b[1] + b[2] + b[3]) * u[1] -
        ...
        dest[3] = α *
                  ...
                  (-6 * (b[1] + b[2] + b[3]) * u[1] +
                   ...
                   4 * (b[4] + b[5]) * u[6] - b[5] * u[7])
        dest[4] = α *
                  inv_left_weights[4] *
                  (4 * (b[1] + b[2] + b[3]) * u[1] -
                   (16 * b[1] + 16 * b[2] + 16 * b[3] + 6 * b[4]) * u[2] +
                   4 * (6 * b[1] + 6 * b[2] + 6 * b[3] + 6 * b[4] + b[5]) * u[3] -
                   (16 * b[1] + 16 * b[2] + 16 * b[3] + 36 * b[4] + 16 * b[5] + b[6]) *
                   u[4] +
                   4 * (b[1] + b[2] + b[3] + 6 * b[4] + 6 * b[5] + b[6]) * u[5] -
                   (6 * b[4] + 16 * b[5] + 6 * b[6]) * u[6] + 4 * (b[5] + b[6]) * u[7] -
                   b[6] * u[8])
        dest[5] = α *
                  inv_left_weights[5] *
                  (-(b[1] + b[2] + b[3]) * u[1] + 4 * (b[1] + b[2] + b[3] + b[4]) * u[2] -
        ...
        dest[end - 1] = α *
                        ...
                        (4 * (b[end - 2] + b[end - 1] + b[end]) * u[end] -
                         (b[end - 3] + 16 * b[end - 2] + 16 * b[end - 1] + 16 * b[end]) *
                         u[end - 1] +
                         4 * (b[end - 3] + 6 * b[end - 2] + 6 * b[end - 1] + 6 * b[end]) *
                         u[end - 2] -
                         (6 * b[end - 3] + 16 * b[end - 2] + 16 * b[end - 1] + 16 * b[end]) *
                         u[end - 3] +
        ...
        dest[end - 2] = α *
                        ...
                        (-6 * (b[end - 2] + b[end - 1] + b[end]) * u[end] +
                         4 * (b[end - 3] + 6 * b[end - 2] + 6 * b[end - 1] + 6 * b[end]) *
                         u[end - 1] -
                         (b[end - 4] + 16 * b[end - 3] + 36 * b[end - 2] + 36 * b[end - 1] +
                          36 * b[end]) *
                         u[end - 2] +
                         4 *
                         (b[end - 4] + 6 * b[end - 3] + 6 * b[end - 2] + 6 * b[end - 1] +
                          6 * b[end]) *
                         u[end - 3] -
                         (6 * b[end - 4] + 16 * b[end - 3] + 6 * b[end - 2] +
                          6 * b[end - 1] + 6 * b[end]) *
                         u[end - 4] + 4 * (b[end - 4] + b[end - 3]) * u[end - 5] -
                         b[end - 4] * u[end - 6])
        dest[end - 3] = α *
                        inv_right_weights[4] *
                        (4 * (b[end - 2] + b[end - 1] + b[end]) * u[end] -
                         ...
                         (b[end - 5] +
                          ...
                          16 * b[end]) * u[end - 3] +
                         4 *
                         (b[end - 5] + 6 * b[end - 4] + 6 * b[end - 3] + b[end - 2] +
                          b[end - 1] + b[end]) *
                         u[end - 4] -
        ...
        dest[end - 4] = α *
                        ...
                        (-(b[end - 2] + b[end - 1] + b[end]) * u[end] +
                         ...
                         (b[end - 6] +
                          ...
                          b[end - 1] +
                          b[end]) * u[end - 4] +
                         4 * (b[end - 6] + 6 * b[end - 5] + 6 * b[end - 4] + b[end - 3]) *
                         u[end - 5] -
        ...
        for i in 5:(length(inv_right_weights) - 1)
            dest[end - i] = α *
                            ...
                            (-b[end - (i + 2)] * u[end - (i + 4)] +
                             ...
                             (6 * b[end - (i + 2)] + 16 * b[end - (i + 1)] + 6 * b[end - i]) *
                             u[end - (i + 2)] +
                             4 *
                             (b[end - (i + 2)] + 6 * b[end - (i + 1)] + 6 * b[end - i] +
                              b[end - (i - 1)]) *
                             u[end - (i + 1)] -
                             (b[end - (i + 2)] +
                              16 * b[end - (i + 1)] +
                              ...
                              b[end - (i - 2)]) * u[end - i] +
                             4 *
                             (b[end - (i + 1)] + 6 * b[end - i] + 6 * b[end - (i - 1)] +
                              b[end - (i - 2)]) *
                             u[end - (i - 1)] -
                             (6 * b[end - i] + 16 * b[end - (i - 1)] + 6 * b[end - (i - 2)]) *
                             u[end - (i - 2)] +
    ...
    @inbounds begin
        ...
        dest[2] = α *
                  ...
                  (+4 * (b[1] + b[2] + b[3]) * u[1] -
                   ...
                   4 * (6 * b[1] + 6 * b[2] + 6 * b[3] + b[4]) * u[3] -
                   (16 * b[1] + 16 * b[2] + 16 * b[3] + 6 * b[4]) * u[4] +
                   4 * (b[1] + b[2] + b[3] + b[4]) * u[5] - b[4] * u[6]) + β * dest[2]
        dest[3] = α *
                  inv_left_weights[3] *
                  (-6 * (b[1] + b[2] + b[3]) * u[1] +
                   4 * (6 * b[1] + 6 * b[2] + 6 * b[3] + b[4]) * u[2] -
                   ...
                   4 * (6 * b[1] + 6 * b[2] + 6 * b[3] + 6 * b[4] + b[5]) * u[4] -
                   (6 * b[1] + 6 * b[2] + 6 * b[3] + 16 * b[4] + 6 * b[5]) * u[5] +
                   4 * (b[4] + b[5]) * u[6] - b[5] * u[7]) + β * dest[3]
        dest[4] = α *
                  inv_left_weights[4] *
                  (4 * (b[1] + b[2] + b[3]) * u[1] -
                   (16 * b[1] + 16 * b[2] + 16 * b[3] + 6 * b[4]) * u[2] +
        ...
        dest[end] = α *
                    ...
                    (-(b[end - 2] + b[end - 1] + b[end]) * u[end] +
                     ...
                     6 * (b[end - 2] + b[end - 1] + b[end]) * u[end - 2] +
                     4 * (b[end - 2] + b[end - 1] + b[end]) * u[end - 3] -
                     (b[end - 2] + b[end - 1] + b[end]) * u[end - 4]) + β * dest[end]
        dest[end - 1] = α *
                        inv_right_weights[2] *
                        (4 * (b[end - 2] + b[end - 1] + b[end]) * u[end] -
                         (b[end - 3] + 16 * b[end - 2] + 16 * b[end - 1] + 16 * b[end]) *
        ...
        for i in 5:(length(inv_right_weights) - 1)
            dest[end - i] = α *
                            ...
                            (-b[end - (i + 2)] * u[end - (i + 4)] +
                             ...
                             u[end - (i - 2)] +
                             4 * (b[end - (i - 1)] + b[end - (i - 2)]) * u[end - (i - 3)] -
                             b[end - (i - 2)] * u[end - (i - 4)]) + β * dest[end - i]
        end
    end
end
function convolve_boundary_coefficients!(dest::AbstractVector,
...
@inline function convolve_interior_coefficients_loopbody(i,
    ...
    @inbounds begin
        ...
        retval = (-b_im2 * u[i - 4] + 4 * (b_im2 + b_im1) * u[i - 3] -
...
function first_derivative_coefficients(source::MattssonSvärdNordström2004,
                                       order::Int,
                                       T = Float64,
                                       mode = FastMode())
    if order == 2
...
function second_derivative_coefficients(source::MattssonSvärdNordström2004,
                                        order::Int,
                                        T = Float64,
                                        mode = FastMode())
    if order == 2
        left_boundary = (
                         # d1
                         DerivativeCoefficientRow{T, 1, 3}(SVector(T(1), T(-2), T(1))),)
        right_boundary = left_boundary
        ...
        left_weights = SVector(T(1 // 2))
        right_weights = left_weights
        left_boundary_derivatives = (DerivativeCoefficientRow{T, 1, 3}(SVector(T(-3 // 2),
                                                                               T(2),
                                                                               T(-1 // 2))),)
    ...
    elseif order == 4
        left_boundary = (
                         #TODO:
                         # d1
                         DerivativeCoefficientRow{T, 1, 4}(SVector(T(2), T(-5), T(4),
                                                                   T(-1))),
                         # d2
                         DerivativeCoefficientRow{T, 1, 3}(SVector(T(1), T(-2), T(1))),
                         # d3
                         DerivativeCoefficientRow{T, 1, 5}(SVector(T(-4 // 43), T(59 // 43),
                                                                   T(-110 // 43),
                                                                   T(59 // 43),
                                                                   T(-4 // 43))),
                         # d4
    ...
    elseif order == 6
        ...
        left_weights = SVector(T(13649 // 43200),
                               ...
                               T(43801 // 43200))
        right_weights = left_weights
        left_boundary_derivatives = (DerivativeCoefficientRow{T, 1, 5}(SVector(T(-25 // 12),
                                                                               T(4), T(-3),
                                                                               T(4 // 3),
                                                                               T(-1 // 4))),)
        right_boundary_derivatives = (-left_boundary_derivatives[1],)
    ...
    elseif order == 8
        left_boundary = (
                         # d1
                         DerivativeCoefficientRow{T, 1, 7}(SVector(T(4870382994799 //
                                                                     1358976868290),
                                                                   T(-893640087518 //
                                                                     75498714905),
                                                                   T(926594825119 //
                                                                   ...
                                                                   T(-451560522577 //
                                                                     2717953736580))),
                         # d2
                         DerivativeCoefficientRow{T, 1, 8}(SVector(T(333806012194 //
                                                                     390619153855),
                                                                   T(-154646272029 //
                                                                     111605472530),
                                                                   T(1168338040 //
                                                                   ...
                                                                   T(-2598164715 //
                                                                     312495323084))),
                         # d3
                         DerivativeCoefficientRow{T, 1, 8}(SVector(T(7838984095 //
                                                                     52731029988),
                                                                   T(1168338040 //
                                                                     5649753213),
                                                                   ...
                                                                   T(10496900965 //
                                                                     39548272491))),
                         # d4
                         DerivativeCoefficientRow{T, 1, 8}(SVector(T(-94978241528 //
                                                                     828644350023),
                                                                   T(82699112501 //
                                                                     157837019052),
                                                                   T(1270761693 //
                                                                   ...
                                                                   T(-44430275135 //
                                                                     552429566682))),
                         # d5
                         DerivativeCoefficientRow{T, 1, 9}(SVector(T(1455067816 //
                                                                     21132528431),
                                                                   T(-171562838 //
                                                                     3018932633),
                                                                   T(-43205598281 //
                                                                   ...
                                                                   T(-1296 // 299527))),
                         # d6
                         DerivativeCoefficientRow{T, 1, 10}(SVector(T(10881504334 //
                                                                      327321118845),
                                                                    T(-28244698346 //
                                                                      140280479505),
                                                                    T(4876378562 //
                                                                    ...
                                                                    T(-144 // 103097))),
                         # d7
                         DerivativeCoefficientRow{T, 1, 11}(SVector(T(-135555328849 //
                                                                      8509847458140),
                                                                    T(11904122576 //
                                                                      101307707835),
                                                                    T(-5124426509 //
        ...
        left_weights = SVector(T(1498139 // 5080320),
                               ...
                               T(5127739 // 5080320))
        right_weights = left_weights
        left_boundary_derivatives = (DerivativeCoefficientRow{T, 1, 7}(SVector(T(-4723 //
                                                                                 2100),
                                                                               T(839 // 175),
                                                                               T(-157 // 35),
                                                                               T(278 // 105),
