...
function dissipation_coefficients(
    ...
    if order == 2
        coefficient_cache =
            MattssonSvärdNordström2004Cache2(inv_left_weights, inv_right_weights)
        b = fill(one(T), length(grid))
        b[1] = T(0)
    elseif order == 4
        coefficient_cache =
            MattssonSvärdNordström2004Cache4(inv_left_weights, inv_right_weights)
        b = fill(one(T), length(grid))
        b[1] = b[end] = T(0)
    elseif order == 6
        coefficient_cache =
            MattssonSvärdNordström2004Cache6(inv_left_weights, inv_right_weights)
        b = fill(one(T), length(grid))
        b[1] = b[2] = b[end] = T(0)
    elseif order == 8
        coefficient_cache =
            MattssonSvärdNordström2004Cache8(inv_left_weights, inv_right_weights)
        b = fill(one(T), length(grid))
...
struct MattssonSvärdNordström2004Cache2{T,LeftWidth,RightWidth} <:
    ...
    function MattssonSvärdNordström2004Cache2(
        inv_left_weights::SVector{LeftWidth,T},
        inv_right_weights::SVector{RightWidth,T},
    ) where {T,LeftWidth,RightWidth}
        boundary_length = 2

        if length(inv_left_weights) < boundary_length
            inv_left_weights = SVector(
                inv_left_weights...,
                ntuple(j -> one(T), boundary_length - length(inv_left_weights))...,
            )
        end
        if length(inv_right_weights) < boundary_length
            inv_right_weights = SVector(
                inv_right_weights...,
                ntuple(j -> one(T), boundary_length - length(inv_right_weights))...,
            )
        end

        new{T,length(inv_left_weights),length(inv_right_weights)}(
            inv_left_weights,
            inv_right_weights,
        )
    end
...
function convolve_boundary_coefficients!(
    dest::AbstractVector,
    ...
    @inbounds begin
        ...
        for i = 3:length(inv_left_weights)
            dest[i] =
                α *
                inv_left_weights[i] *
        ...
        dest[end-1] =
            α *
            inv_right_weights[2] *
            (-b[end-1] * u[end-2] + (b[end-1] + b[end]) * u[end-1] - b[end] * u[end])
        for i = 2:length(inv_right_weights)-1
            dest[end-i] =
                α *
                inv_right_weights[i+1] *
                (
                    -b[end-i] * u[end-(i-1)] + (b[end-i] + b[end-(i+1)]) * u[end-i] -
                    b[end-(i+1)] * u[end-(i+1)]
                )
        end
    end
...
function convolve_boundary_coefficients!(
    ...
    @inbounds begin
        dest[1] =
            α * inv_left_weights[1] * ((b[1] + b[2]) * u[1] + (b[1] - b[2]) * u[2]) +
            β * dest[1]
        dest[2] =
            α *
            inv_left_weights[2] *
            ((b[1] - b[2]) * u[1] + (b[1] + b[2] + b[3]) * u[2] - b[3] * u[3]) + β * dest[2]
        for i = 3:length(inv_left_weights)
            dest[i] =
                α *
                inv_left_weights[i] *
                (-b[i] * u[i-1] + (b[i] + b[i+1]) * u[i] - b[i+1] * u[i+1]) + β * dest[i]
        end

        dest[end] =
            α * inv_right_weights[1] * (b[end] * u[end] - b[end] * u[end-1]) + β * dest[end]
        dest[end-1] =
            α *
            inv_right_weights[2] *
            (-b[end-1] * u[end-2] + (b[end-1] + b[end]) * u[end-1] - b[end] * u[end]) +
...
@inline function convolve_interior_coefficients_loopbody(
    i,
    cache::MattssonSvärdNordström2004Cache2,
    u,
    b,
...
struct MattssonSvärdNordström2004Cache4{T,LeftWidth,RightWidth} <:
    ...
    function MattssonSvärdNordström2004Cache4(
        inv_left_weights::SVector{LeftWidth,T},
        inv_right_weights::SVector{RightWidth,T},
    ) where {T,LeftWidth,RightWidth}
        boundary_length = 3
...
function convolve_boundary_coefficients!(
    ...
    @inbounds begin
        ...
        for i = 4:length(inv_left_weights)
            dest[i] =
                α *
                inv_left_weights[i] *
                (
                    -b[i-1] * u[i-2] + 2 * (b[i-1] + b[i]) * u[i-1] -
                    (b[i-1] + 4 * b[i] + b[i+1]) * u[i] + 2 * (b[i] + b[i+1]) * u[i+1] -
                    b[i+1] * u[i+2]
                )
        end
        ...
        dest[end] =
            ...
            inv_right_weights[1] *
            (
                -(b[end] + b[end-1]) * u[end] + 2 * (b[end] + b[end-1]) * u[end-1] -
        ...
        dest[end-1] =
            ...
            (
                ...
                2 * (b[end-2] + b[end-1] + b[end]) * u[end-2] - b[end-2] * u[end-3]
            )
        dest[end-2] =
            α *
            inv_right_weights[3] *
            (
                ...
                2 * (b[end-3] + b[end-2]) * u[end-3] - b[end-3] * u[end-4]
            )
        for i = 3:length(inv_right_weights)-1
            dest[end-i] =
                α *
                inv_right_weights[i+1] *
                (
...
@inline function convolve_interior_coefficients_loopbody(
    ...
    @inbounds begin
        ...
        retval = (
            -b_im1 * u[i-2] + 2 * (b_im1 + b_i) * u[i-1] - (b_im1 + 4b_i + b_ip1) * u[i] + 2 * (b_i + b_ip1) * u[i+1] - b_ip1 * u[i+2]
        )
    end
...
struct MattssonSvärdNordström2004Cache6{T,LeftWidth,RightWidth} <:
    ...
    function MattssonSvärdNordström2004Cache6(
        inv_left_weights::SVector{LeftWidth,T},
        inv_right_weights::SVector{RightWidth,T},
    ) where {T,LeftWidth,RightWidth}
        boundary_length = 4
...
function convolve_boundary_coefficients!(
    ...
    @inbounds begin
        ...
        for i = 5:length(inv_left_weights)
            dest[i] =
                α *
                inv_left_weights[i] *
                (
        ...
        dest[end] =
            ...
            inv_right_weights[1] *
            (
                -(b[end] + b[end-1]) * u[end] + 3 * (b[end] + b[end-1]) * u[end-1] -
                3 * (b[end] + b[end-1]) * u[end-2] + (b[end] + b[end-1]) * u[end-3]
            )
        dest[end-1] =
            α *
            inv_right_weights[2] *
            (
                ...
                3 * (b[end-2] + b[end-1] + b[end]) * u[end-3] + b[end-2] * u[end-4]
            )
        dest[end-2] =
            α *
            inv_right_weights[3] *
            (
                ...
                3 * (b[end-3] + b[end-2]) * u[end-4] + b[end-3] * u[end-5]
            )
        dest[end-3] =
            α *
            inv_right_weights[4] *
            (
                ...
                3 * (b[end-4] + b[end-3]) * u[end-5] + b[end-4] * u[end-6]
            )
        for i = 4:length(inv_right_weights)-1
            dest[end-i] =
                α *
                inv_right_weights[i+1] *
                (
                    ...
                    (b[end-(i+1)] + 9 * b[end-i] + 9 * b[end-(i-1)] + b[end-(i-2)]) *
                    u[end-i] +
...
@inline function convolve_interior_coefficients_loopbody(
    ...
    @inbounds begin
        ...
        retval = (
            b_im1 * u[i-3] - 3 * (b_im1 + b_i) * u[i-2] +
            3 * (b_im1 + 3 * b_i + b_ip1) * u[i-1] -
            (b_im1 + 9 * b_i + 9 * b_ip1 + b_ip2) * u[i] +
            3 * (b_i + 3 * b_ip1 + b_ip2) * u[i+1] - 3 * (b_ip1 + b_ip2) * u[i+2] +
            b_ip2 * u[i+3]
        )
    end
...
struct MattssonSvärdNordström2004Cache8{T,LeftWidth,RightWidth} <:
    ...
    function MattssonSvärdNordström2004Cache8(
        inv_left_weights::SVector{LeftWidth,T},
        inv_right_weights::SVector{RightWidth,T},
    ) where {T,LeftWidth,RightWidth}
        boundary_length = 5
...
function convolve_boundary_coefficients!(
    ...
    @inbounds begin
        ...
        for i = 6:length(inv_left_weights)
            dest[i] =
                α *
                inv_left_weights[i] *
                (
        ...
        dest[end-2] =
            ...
            (
                ...
                (b[end-4] + 16 * b[end-3] + 36 * b[end-2] + 36 * b[end-1] + 36 * b[end]) *
                u[end-2] +
                4 *
                (b[end-4] + 6 * b[end-3] + 6 * b[end-2] + 6 * b[end-1] + 6 * b[end]) *
                u[end-3] -
                (6 * b[end-4] + 16 * b[end-3] + 6 * b[end-2] + 6 * b[end-1] + 6 * b[end]) *
                u[end-4] + 4 * (b[end-4] + b[end-3]) * u[end-5] - b[end-4] * u[end-6]
            )
        dest[end-3] =
            α *
            inv_right_weights[4] *
            (
                ...
                4 *
                (b[end-5] + 6 * b[end-4] + 6 * b[end-3] + b[end-2] + b[end-1] + b[end]) *
                u[end-4] - (6 * b[end-5] + 16 * b[end-4] + 6 * b[end-3]) * u[end-5] +
                4 * (b[end-5] + b[end-4]) * u[end-6] - b[end-5] * u[end-7]
            )
        dest[end-4] =
            α *
            inv_right_weights[5] *
            (
                ...
                4 * (b[end-6] + b[end-5]) * u[end-7] - b[end-6] * u[end-8]
            )
        for i = 5:length(inv_right_weights)-1
            dest[end-i] =
                α *
                inv_right_weights[i+1] *
                (
                    ...
                    4 *
                    (b[end-(i+2)] + 6 * b[end-(i+1)] + 6 * b[end-i] + b[end-(i-1)]) *
                    u[end-(i+1)] -
                    (
                        b[end-(i+2)] +
                        16 * b[end-(i+1)] +
                    ...
                    4 *
                    (b[end-(i+1)] + 6 * b[end-i] + 6 * b[end-(i-1)] + b[end-(i-2)]) *
                    u[end-(i-1)] -
...
@inline function convolve_interior_coefficients_loopbody(
    ...
    @inbounds begin
        ...
        retval = (
            -b_im2 * u[i-4] + 4 * (b_im2 + b_im1) * u[i-3] -
            ...
            (6 * b_i + 16 * b_ip1 + 6 * b_ip2) * u[i+2] + 4 * (b_ip1 + b_ip2) * u[i+3] -
            b_ip2 * u[i+4]
        )
    end
    ...
    elseif order == 4
        left_boundary = (
            ...
            DerivativeCoefficientRow{T,1,4}(
                SVector(T(-24 // 17), T(59 // 34), T(-4 // 17), T(-3 // 34)),
            ),
            # q2
            DerivativeCoefficientRow{T,1,3}(SVector(T(-1 // 2), T(0), T(1 // 2))),
            # q3
            DerivativeCoefficientRow{T,1,5}(
                SVector(T(4 // 43), T(-59 // 86), T(0), T(59 // 86), T(-4 // 43)),
            ),
            # q4
            DerivativeCoefficientRow{T,1,6}(
                SVector(T(3 // 98), T(0), T(-59 // 98), T(0), T(32 // 49), T(-4 // 49)),
            ),
        )
        right_boundary = .-left_boundary
...
function second_derivative_coefficients(
    source::MattssonSvärdNordström2004,
    order::Int,
    T = Float64,
    mode = FastMode(),
    ...
    if order == 2
        ...
        right_weights = left_weights
        left_boundary_derivatives =
            (DerivativeCoefficientRow{T,1,3}(SVector(T(-3 // 2), T(2), T(-1 // 2))),)
    ...
    elseif order == 4
        left_boundary = (
            #TODO:
            # d1
            DerivativeCoefficientRow{T,1,4}(SVector(T(2), T(-5), T(4), T(-1))),
            # d2
            DerivativeCoefficientRow{T,1,3}(SVector(T(1), T(-2), T(1))),
            # d3
            DerivativeCoefficientRow{T,1,5}(
                SVector(T(-4 // 43), T(59 // 43), T(-110 // 43), T(59 // 43), T(-4 // 43)),
            ),
            # d4
            DerivativeCoefficientRow{T,1,6}(
                SVector(
        ...
        left_boundary_derivatives = (
            DerivativeCoefficientRow{T,1,4}(
                SVector(T(-11 / 6), T(3), T(-3 // 2), T(1 // 3)),
            ),
        )
        right_boundary_derivatives = (-left_boundary_derivatives[1],)
    ...
    elseif order == 6
        ...
        left_boundary_derivatives = (
            DerivativeCoefficientRow{T,1,5}(
                SVector(T(-25 // 12), T(4), T(-3), T(4 // 3), T(-1 // 4)),
            ),
        )
        right_boundary_derivatives = (-left_boundary_derivatives[1],)
