...
function third_derivative_coefficients(
    source::Mattsson2014,
    order::Int,
    T = Float64,
    mode = FastMode(),
    ...
    if order == 2
        left_boundary = (
            # d1
            DerivativeCoefficientRow{T,1,4}(
                SVector(T(1 // 4), T(-5 // 8), T(1 // 2), T(-1 // 8)),
            ),
            # d2
            DerivativeCoefficientRow{T,1,4}(
                SVector(T(-11 // 16), T(2), T(-31 // 16), T(5 // 8)),
            ),
            # d3
            DerivativeCoefficientRow{T,1,5}(
                SVector(T(-1 // 2), T(15 // 16), T(1 // 8), T(-17 // 16), T(1 // 2)),
            ),
        )
        right_boundary = .-left_boundary
        ...
        right_boundary_derivatives =
            (-left_boundary_derivatives[1], left_boundary_derivatives[2])
    ...
    elseif order == 6
        ...
        left_boundary_derivatives = (
            ...
            # second derivative
            DerivativeCoefficientRow{T,1,5}(
                SVector(T(35 // 12), T(-26 // 3), T(19 // 2), T(-14 // 3), T(11 // 12)),
            ),
        )
        right_boundary_derivatives =
            (-left_boundary_derivatives[1], left_boundary_derivatives[2])
...
function fourth_derivative_coefficients(
    source::Mattsson2014,
    order::Int,
    T = Float64,
    mode = FastMode(),
    ...
    if order == 2
        left_boundary = (
            # d1
            DerivativeCoefficientRow{T,1,4}(
                SVector(T(8 // 5), T(-24 // 5), T(24 // 5), T(-8 // 5)),
            ),
            # d2
            DerivativeCoefficientRow{T,1,4}(
                SVector(T(-2 // 5), T(6 // 5), T(-6 // 5), T(2 // 5)),
            ),
            # d3
            DerivativeCoefficientRow{T,1,5}(
                SVector(T(2 // 5), T(-11 // 5), T(21 // 5), T(-17 // 5), T(1)),
            ),
            # d4
            DerivativeCoefficientRow{T,1,6}(
                SVector(T(1 // 5), T(2 // 5), T(-17 // 5), T(29 // 5), T(-4), T(1)),
            ),
        )
        right_boundary = left_boundary
        ...
        left_boundary_derivatives = (
            ...
            DerivativeCoefficientRow{T,1,3}(SVector(T(1), T(-2), T(1))),
            # third derivative
            DerivativeCoefficientRow{T,1,4}(SVector(T(-1), T(3), T(-3), T(1))),
        )
        right_boundary_derivatives = (
    ...
    elseif order == 6
        ...
        left_boundary_derivatives = (
            ...
            # third derivative
            DerivativeCoefficientRow{T,1,5}(
                SVector(T(-5 // 2), T(9), T(-12), T(7), T(-3 // 2)),
            ),
        )
        right_boundary_derivatives = (
            -left_boundary_derivatives[1],
