...
@auto_hash_equals struct VarCoefDerivativeCoefficients{T,
                                                       ...
                                                       LeftWidth, RightWidth, ExecutionMode,
    ...
    function VarCoefDerivativeCoefficients(coefficient_cache::CoefficientCache,
        ...
        new{T, CoefficientCache, LeftWidth, RightWidth, ExecutionMode,
            SourceOfCoefficients}(coefficient_cache, left_weights, right_weights, mode,
                                  derivative_order, accuracy_order, source_of_coefficients)
    end
end
...
function dissipation_operator(source_of_coefficients, order, xmin, xmax, N,
    ...
    coefficients, b = dissipation_coefficients(source_of_coefficients, order, grid,
                                               left_weights, right_weights, mode)
...
function dissipation_operator(source_of_coefficients, D::DerivativeOperator{T};
    ...
    x = grid(D)
    dissipation_operator(source_of_coefficients, order, first(x), last(x), length(x),
                         D.coefficients.left_weights, D.coefficients.right_weights,
                         strength, mode)
end
