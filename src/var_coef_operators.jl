...
"""
@auto_hash_equals struct VarCoefDerivativeOperator{
    T,
    CoefficientCache,
    ExecutionMode,
    ...
    function VarCoefDerivativeOperator(
        coefficients::VarCoefDerivativeCoefficients{
            T,
            CoefficientCache,
            ExecutionMode,
        ...
        grid::Grid,
        b::Vector{T},
    ) where {T,CoefficientCache,ExecutionMode,SourceOfCoefficients,Grid}
...
`mode=ThreadedMode()`.
"""
function var_coef_derivative_operator(
    ...
    coefficients = var_coef_derivative_coefficients(
        source_of_coefficients,
        derivative_order,
        accuracy_order,
        grid,
...
function scale_by_mass_matrix!(
    ...
    @simd for i = length(left_weights)+1:N-length(right_weights)
