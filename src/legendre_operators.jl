...
@auto_hash_equals struct LegendreDerivativeOperator{T <: Real} <:
    ...
    function LegendreDerivativeOperator(xmin::T, xmax::T,
...
function scale_by_mass_matrix!(u::AbstractVector,
                               D::Union{LegendreDerivativeOperator,
                                        LegendreSecondDerivativeOperator}, factor = true)
...
function Base.show(io::IO, D::LegendreDerivativeOperator)
    if get(io, :compact, false)
        ...
        print(io, " on ", length(x), " Lobatto Legendre nodes in [", first(x), ", ",
              last(x), "]")
    end
end
...
function mul!(dest::AbstractVector, D::LegendreDerivativeOperator, u::AbstractVector,
              α = true, β = false)
    @unpack jac, basis = D
    N, _ = size(D)
    @boundscheck begin
...
function mul!(dest::AbstractVector, D::LegendreSecondDerivativeOperator, u::AbstractVector,
              α = true, β = false)
    @unpack jac, D2 = D
    N, _ = size(D2)
    @boundscheck begin
...
function ConstantFilter(basis::LobattoLegendre{T}, filter,
