...
`Val(:NonReflecting)`.
"""
@auto_hash_equals struct WaveEquationNonperiodicSemidiscretization{Derivative <:
...
function semidiscretize(du0func, u0func, semi::WaveEquationNonperiodicSemidiscretization,
                        tspan)
...
function (disc::WaveEquationNonperiodicSemidiscretization)(ddu, du, u, p, t)
    ...
    if left_bc == Val(:HomogeneousNeumann)
        @inbounds ddu[1] += derivative_left(derivative, u, Val(1)) /
                            left_boundary_weight(derivative)
    elseif left_bc == Val(:HomogeneousDirichlet)
        mul_transpose_derivative_left!(ddu, derivative, Val(1),
                                       -u[1] / left_boundary_weight(derivative), true)
        ...
        @inbounds ddu[1] -= accuracy_order(derivative) * u[1] /
                            left_boundary_weight(derivative)^2
    elseif left_bc == Val(:NonReflecting)
        @inbounds ddu[1] -= (du[1] - derivative_left(derivative, u, Val(1))) /
                            left_boundary_weight(derivative)
    else
    ...
    if right_bc == Val(:HomogeneousNeumann)
        @inbounds ddu[end] -= derivative_right(derivative, u, Val(1)) /
                              right_boundary_weight(derivative)
    elseif right_bc == Val(:HomogeneousDirichlet)
        mul_transpose_derivative_right!(ddu, derivative, Val(1),
                                        u[end] / right_boundary_weight(derivative), true)
        # only a rough estimate, should probably be improved, see above
        @inbounds ddu[end] -= accuracy_order(derivative) * u[end] /
                              right_boundary_weight(derivative)^2
    elseif right_bc == Val(:NonReflecting)
        @inbounds ddu[end] -= (du[end] + derivative_right(derivative, u, Val(1))) /
                              right_boundary_weight(derivative)
    else
