...
"""
@auto_hash_equals struct MatrixDerivativeOperator{T, Dtype <: AbstractMatrix{T},
    ...
    function MatrixDerivativeOperator(xmin::T, xmax::T,
        ...
        new{T, typeof(D), SourceOfCoefficients}(grid, Δx * weights, jac * D, accuracy_order,
                                                source)
    end
end
...
function scale_by_mass_matrix!(u::AbstractVector, D::AbstractMatrixDerivativeOperator,
                               factor = true)
...
function scale_by_inverse_mass_matrix!(u::AbstractVector,
                                       D::AbstractMatrixDerivativeOperator, factor = true)
    Base.require_one_based_indexing(u)
    @boundscheck begin
        length(u) == size(D, 2) ||
            throw(DimensionMismatch("sizes of input vector and operator do not match"))
...
function mul!(dest::AbstractVector, Dop::MatrixDerivativeOperator, u::AbstractVector,
              α = true, β = false)
    @unpack D = Dop
    N, _ = size(D)
    @boundscheck begin
