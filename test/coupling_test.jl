    ...
    for T in (Float32, Float64, Rational{Int128})
        for acc_order in 2:7
            ...
            for N in 1:3
                ...
                xmax = T(2)
                mesh = UniformMesh1D(xmin, xmax, N)
                cDp_continuous = couple_continuously(Dp, mesh)
                cDm_continuous = couple_continuously(Dm, mesh)
                ...
                for cD in (cDp_continuous, cDm_continuous, cDp_central, cDm_central,
                           cDp_plus, cDm_minus)
                    print(cD)
                    x = grid(cD)
                    println(x)
                ...
                for (cDp, cDm) in ((cDp_continuous, cDm_continuous),
                                   (cDp_central, cDm_central), (cDp_plus, cDm_minus))
