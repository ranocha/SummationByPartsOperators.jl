...
for source in D_test_list, T in (Float32, Float64)
    ...
    if D !== nothing
        ...
        # integration
        k = 0
        @test integrate(x0, D) ≈ (xmax^(k + 1) - (xmin)^(k + 1)) / (k + 1)
        k = 1
        @test integrate(x1, D) ≈ (xmax^(k + 1) - (xmin)^(k + 1)) / (k + 1)
    ...
    if D !== nothing
        ...
        @test integrate(x1, D) ≈ (xmax^(k + 1) - (xmin)^(k + 1)) / (k + 1)
        k = 2
        @test integrate(x2, D) ≈ (xmax^(k + 1) - (xmin)^(k + 1)) / (k + 1)
        k = 3
        @test integrate(x3, D) ≈ (xmax^(k + 1) - (xmin)^(k + 1)) / (k + 1)
        @test integrate(identity, x1, D) ≈ (xmax^2 - xmin^2) / 2
        # boundary integration
        @test integrate_boundary(x0, D) ≈ x0[end] - x0[begin]
        @test integrate_boundary(x1, D) ≈ x1[end] - x1[begin]
    ...
    if D !== nothing
        ...
        @test integrate(x3, D) ≈ (xmax^(k + 1) - (xmin)^(k + 1)) / (k + 1)
        k = 4
        @test integrate(x4, D) ≈ (xmax^(k + 1) - (xmin)^(k + 1)) / (k + 1)
        k = 5
        @test integrate(x5, D) ≈ (xmax^(k + 1) - (xmin)^(k + 1)) / (k + 1)
        k = 6
        @test integrate(x6, D) ≈ (xmax^(k + 1) - (xmin)^(k + 1)) / (k + 1)
        k = 7
        @test integrate(x7, D) ≈ (xmax^(k + 1) - (xmin)^(k + 1)) / (k + 1)
        @test integrate(identity, x1, D) ≈ (xmax^2 - xmin^2) / 2
        # boundary integration
        @test integrate_boundary(x0, D) ≈ x0[end] - x0[begin]
        @test integrate_boundary(x1, D) ≈ x1[end] - x1[begin]
    ...
    @test_throws Union{MethodError, ArgumentError} derivative_operator(source, der_order,
                                                                       16, xmin, xmax, N)
end
