...
let T = Float32
    ...
    derivative_order = 1
    accuracy_order = 2
    D = periodic_central_derivative_operator(derivative_order, accuracy_order, xmin, xmax,
                                             N)
    ...
    @test all(i -> abs(res[i]) < eps(T), accuracy_order:(length(res) - accuracy_order))
    mul!(res, D, x1)
    @test all(i -> abs(res[i]) < 100N * eps(T),
              accuracy_order:(length(res) - accuracy_order))
    mul!(res, D, x2)
    @test all(i -> isapprox(res[i], 2 * x0[i], atol = 620N * eps(T)),
              accuracy_order:(length(res) - accuracy_order))
    mul!(res, D, x3)
    @test all(i -> isapprox(res[i], 6 * x1[i], atol = 2000N * eps(T)),
              accuracy_order:(length(res) - accuracy_order))
    mul!(res, D, x4)
    ...
    @test M * Matrix(D) - Matrix(D)' * M ≈ zeros(T, N, N)
    mul!(res, D, x0)
    @test all(i -> abs(res[i]) < 50N * eps(T),
              accuracy_order:(length(res) - accuracy_order))
    mul!(res, D, x1)
    @test all(i -> abs(res[i]) < 1000N * eps(T),
              accuracy_order:(length(res) - accuracy_order))
    mul!(res, D, x2)
    @test all(i -> isapprox(res[i], 2 * x0[i], atol = 5000N * eps(T)),
              accuracy_order:(length(res) - accuracy_order))
    mul!(res, D, x3)
    @test all(i -> isapprox(res[i], 6 * x1[i], atol = 5000N * eps(T)),
              accuracy_order:(length(res) - accuracy_order))
    mul!(res, D, x4)
    @test all(i -> isapprox(res[i], 12 * x2[i], atol = 8500N * eps(T)),
              accuracy_order:(length(res) - accuracy_order))
    mul!(res, D, x5)
    ...
    mul!(res, D, x4)
    @test norm((res - 12 .* x2)[accuracy_order:(end - accuracy_order)], Inf) <
          8000N * eps(T)
    mul!(res, D, x5)
    @test norm((res - 20 .* x3)[accuracy_order:(end - accuracy_order)], Inf) <
          50000N * eps(T)
    mul!(res, D, x6)
    @test norm((res - 30 .* x4)[accuracy_order:(end - accuracy_order)], Inf) <
          52000N * eps(T)
    mul!(res, D, x7)
    @test norm((res - 42 .* x5)[accuracy_order:(end - accuracy_order)], Inf) <
          92000N * eps(T)
    # mass matrix scaling
    M = @inferred mass_matrix(D)
    u = sinpi.(x1)
    ...
    mul!(res, D, x3)
    @test norm((res - 6 .* x0)[accuracy_order:(end - accuracy_order)], Inf) <
          500000N * eps(T)
    mul!(res, D, x4)
    @test norm((res - 24 .* x1)[accuracy_order:(end - accuracy_order)], Inf) <
          600000N * eps(T)
    mul!(res, D, x5)
    @test norm((res - 60 .* x2)[accuracy_order:(end - accuracy_order)], Inf) >
          600000 * eps(T)
    # mass matrix scaling
    M = @inferred mass_matrix(D)
    u = sinpi.(x1)
    ...
    @test norm((res - 60 .* x2)[accuracy_order:(end - accuracy_order)], Inf) <
          4000000N * eps(T)
    mul!(res, D, x6)
    @test norm((res - 120 .* x3)[accuracy_order:(end - accuracy_order)], Inf) <
          8000000N * eps(T)
    mul!(res, D, x7)
    @test norm((res - 240 .* x4)[accuracy_order:(end - accuracy_order)], Inf) >
          220000N * eps(T)
    ...
    accuracy_order = 2
    @test_throws ArgumentError periodic_central_derivative_operator(4, accuracy_order, xmin,
                                                                    xmax, N)
    @test_throws ArgumentError periodic_central_derivative_operator(5, accuracy_order, xmin,
                                                                    xmax, N)
    @test_throws ArgumentError periodic_central_derivative_operator(6, accuracy_order, xmin,
                                                                    xmax, N)
...
let T = Float64
    ...
    @test all(i -> res[i] ≈ 2 * x1[i], accuracy_order:(length(res) - accuracy_order))
    mul!(res, D, x3)
    @test all(i -> isapprox(res[i], 3 * x2[i], atol = 100N * eps(T)),
              accuracy_order:(length(res) - accuracy_order))
    mul!(res, D, x4)
    @test all(i -> res[i] ≈ 4 * x3[i], accuracy_order:(length(res) - accuracy_order))
    mul!(res, D, x5)
    @test all(i -> isapprox(res[i], 5 * x4[i], atol = 100N * eps(T)),
              accuracy_order:(length(res) - accuracy_order))
    mul!(res, D, x6)
    @test all(i -> res[i] ≈ 6 * x5[i], accuracy_order:(length(res) - accuracy_order))
    mul!(res, D, x7)
...
for T in (Float32, Float64), accuracy_order in 2:2:12, derivative_order in 1:3
    ...
    xmax = one(T)
    N = 21
    D_central = periodic_central_derivative_operator(derivative_order, accuracy_order, xmin,
                                                     xmax, N)
    D_general = periodic_derivative_operator(derivative_order, accuracy_order, xmin, xmax,
                                             N)
...
for T in (Float32, Float64), accuracy_order in 1:10, derivative_order in 1:3
    ...
    D_threads = periodic_derivative_operator(derivative_order, accuracy_order, xmin, xmax,
                                             N, ThreadedMode())
    ...
    @test all(i -> isapprox(dest_serial[i], dest_full[i], rtol = 5 * sqrt(eps(T))),
              eachindex(u))
    @test all(i -> isapprox(dest_serial[i], dest_sparse[i], rtol = 5 * sqrt(eps(T))),
              eachindex(u))
end
...
for T in (Float32, Float64), accuracy_order in 1:10, derivative_order in 1:3
    ...
    D_threads = periodic_derivative_operator(derivative_order, accuracy_order, xmin, xmax,
                                             N, ThreadedMode())
    D_safe = periodic_derivative_operator(derivative_order, accuracy_order, xmin, xmax, N,
                                          SafeMode())
...
let T = Float32
    ...
    @test_throws ArgumentError D=periodic_derivative_operator(Holoborodko2008(), der_order,
                                                              acc_order,
                                                              xmin, xmax, N,
                                                              stencil_width = 3)
    @test_throws ArgumentError D=periodic_derivative_operator(Holoborodko2008(), der_order,
    ...
    for stencil_width in 5:2:9
        ...
        @test all(i -> abs(res[i]) < 100N * eps(T),
                  stencil_width:(length(res) - stencil_width))
        mul!(res, D, x2)
        @test all(i -> isapprox(res[i], 2 * x0[i], atol = 600N * eps(T)),
                  stencil_width:(length(res) - stencil_width))
        mul!(res, D, x3)
        @test all(i -> isapprox(res[i], 6 * x1[i], atol = 2000N * eps(T)),
                  stencil_width:(length(res) - stencil_width))
    ...
    for stencil_width in 7:2:9
        ...
        @test all(i -> isapprox(res[i], 6 * x1[i], atol = 5000N * eps(T)),
                  stencil_width:(length(res) - stencil_width))
        mul!(res, D, x4)
        @test all(i -> isapprox(res[i], 12 * x2[i], atol = 7000N * eps(T)),
                  stencil_width:(length(res) - stencil_width))
        mul!(res, D, x5)
...
@testset "LanczosLowNoise" begin
    @testset "T = $T" for T in (Float32, Float64)
        ...
        xmax = 2 * one(T)
        N = 100
        x1 = compute_coefficients(identity,
...
let N = 4
    @test_throws ArgumentError periodic_derivative_operator(1, 2, 0 // 1, (N - 1) // 1, N,
                                                            1)
    D = periodic_derivative_operator(1, 2, 0 // 1, N, N, 0)
    @test Matrix(D) ≈ [-3//2 2//1 -1//2 0//1
           0//1 -3//2 2//1 -1//2
           -1//2 0//1 -3//2 2//1
           2//1 -1//2 0//1 -3//2]
    D = periodic_derivative_operator(1, 2, 0 // 1, N, N, -2)
    @test Matrix(D) ≈ [3//2 0//1 1//2 -2//1
           -2//1 3//2 0//1 1//2
           1//2 -2//1 3//2 0//1
           0//1 1//2 -2//1 3//2]
end
